...
#   RUN_RESTORE_VALIDATE (auto: true for L0, false for L1/ARCH)
...
case "${BACKUP_TYPE}" in
  L0)
    ...
    "
    RUN_RESTORE_VALIDATE="${RUN_RESTORE_VALIDATE:-true}"
    ;;
  L1)
    RMAN_BLOCK="
      SQL 'ALTER SYSTEM ARCHIVE LOG CURRENT';
    ...
    "
    RUN_RESTORE_VALIDATE="${RUN_RESTORE_VALIDATE:-false}"
    ;;
  ARCH)
    RMAN_BLOCK="
      SQL 'ALTER SYSTEM ARCHIVE LOG CURRENT';
...
VALIDATE_BLOCK=""
if [[ "${RUN_RESTORE_VALIDATE}" == "true" ]]; then
  VALIDATE_BLOCK="RESTORE DATABASE VALIDATE;"
fi
...
echo "RMAN ${BACKUP_TYPE} completed (RUN_RESTORE_VALIDATE=${RUN_RESTORE_VALIDATE}). Log: ${LOG_FILE}"
